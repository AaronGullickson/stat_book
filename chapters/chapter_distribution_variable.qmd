...
The frequency we just calculated is sometimes called the **absolute frequency** because it just counts the raw number of observations. However such raw numbers are often not very helpful because they will vary by the overall number of observations. Instead, we usually want to calculate the **proportion** of observations that fall within each category. These proportions are sometimes also called the **relative frequency**.  We can calculate the proportion by simply dividing our absolute frequency by the total number of observations:
...
tab |>
  ...
  summary_rows(columns = c(n, prop, percent),
               fns = list(Total = ~sum(.)),
               formatter = fmt_integer,
               missing_text = "") |>
...
Now that we have proportions/percents, we can use these values to construct a graphical display of the distribution. One of the most common techniques for doing this is a **piechart**. @fig-piechart shows a piechart of the distribution of passengers on the Titanic. 
...
A better way to display the distribution is by using a **barplot** in which vertical or horizontal bars give the proportion or percent. @fig-barplot-passclass shows a barplot for the distribution of passengers on the Titanic. 
...
The ggplot library is a very flexible system that repeats these same basic layer elements in all the graphs it creates. In an appendix to this book, you can see cookbook examples of all the ways we will we use it to create figures throughout the term. 
...
ggplot(politics, aes(x=age))+
  geom_histogram(binwidth=10, col="black", fill="grey", closed="left")+
  labs(x="age")+
  theme_bw()
```

Looking at @fig-hist-agepol, I can see the peak in the distribution in the 55-64 group (the baby boomers) with a long fat tail to the right and a steeper drop off at older ages, due to smaller cohorts and old age mortality. I can also see a smaller peak in the 25-34 range which is largely the children of the baby boomers. 
...
ggplot(politics, aes(x=age))+
  geom_histogram(binwidth=5, col="black", fill="grey", closed="left")+
  labs(x="age")+
  theme_bw()
```
...
When a distribution has two distinct peaks, we call it a **bimodal** distribution. @fig-hist-bimodal-property shows that the distribution of violent crime rates across states is bimodal. The first peak is around 2200-2500 crimes per 100,000 and the second is around 3500 crimes per 100,000. This suggests that there are two distinctive clusters of states: in the first cluster are states with a moderate crime rate and in the second cluster, states with high crime rate states. You can probably guess what we call a distribution with three peaks (and so on), but its fairly rare to see more than two distinct peaks in a distribution.

```{r}
#| label: fig-hist-bimodal-property
#| fig-cap: The distribution of property crimes by states is bimodal with a two separate peaks.
#| echo: false
...
```{r}
#| label: fig-hist-rskewed-boxoffice
#| fig-cap: The distribution of movie box office returns is heavily right skewed
#| echo: false
...
In @fig-hist-bimodal-property above, we can see a clear outlier in the violent crime rate for Washington DC. Washington DC's crime rate is such an outlier relative to the other states that we will pay attention to it throughout this term as we conduct our analysis. @fig-hist-spread-runtime above shows that Peter Jackson's Return of the King was an outlier for the runtime of sci-fi/fantasy movies, although it doesn't seem quite as extreme as for the case of Washington DC. 
...
As an example, lets take a sub-sample of our movie data. I am going to select all the romances (not including rom-coms which are coded as comedies) produced in 2018. There were nine "pure" romances in 2017. I want to know their mean Metacritic score. 
...
movies |> 
  ...
  tab_stubhead("Title") |>
  summary_rows(columns = metascore,
               fns = list(Sum = ~sum(.)),
               missing_text = "",
               formatter = fmt_integer)
```

In the very last row, I show the sum of the Metacritic score which simply sums up the Metacritic score of each movie. To calculate the mean, I simply divide this sum by the number of movies which is 9.

$$\bar{x}=\frac{522}{9}=58$$

The mean Metacritic score for romantic movies in 2017 was 58. Notice the funny $x$ with a bar over it ("x bar"). Mathematically, we often represent variables with lower-case roman letters like $x$ or $y$. Putting the bar above the letter is the way to mathematically signify the mean of that variable. 
...
We can see that the mean Metacritic score of romantic movies in 2017 (58) were slightly below the mean for all the movies in our dataset (`r round(mean(movies$metascore),1)`), but not by much.
...
In order to calculate the median, we need to first order our observations from lowest to highest. Lets do that with the 2017 romantic movie data from before. 
...
In this particular case, the mean (`r round(mean(movies$metascore),1)`) and median (`r round(median(movies$metascore),1)`) of the Metacritic score produce very similar measures of center, but this isn't always the case as I will demonstrate below. 

As for the mean, it makes no sense to calculate the median of a categorical variable.

### The mode

The mode is the least used of the three measures of center. In fact, it is so infrequently used that R does not even have a built-in function to calculate it. The mode is the **high point** or **peak** of the distribution. When you look at a distribution graphically, the mode is what your eye is drawn to as a measure of center. Calculating the mode however is much trickier. Simply speaking, the mode is the most common value in the data. However, when data are recorded down to very precise decimal levels, this can be a misleading number. Furthermore, there may be multiple "peaks" in the data and so speaking of a single mode can be misleading. In the sample of 2018 romance movies, no value was repeated and so there is no legitimate value for the mode. 

In the case of the Metacritic score distribution for the entire movies dataset, the single most common value was a score of 49 which was received by 109 movies. The score of 55 was a close second at 105 movies. @fig-hist-metacritic shows a histogram with binwidths of one. Although the score of 49 is the single highest value, you can see that it is surrounded by a group of high values. Its probably more accurate to describe the peak as "around 50" than a precise number.

```{r hist-tomato-meter, fig.cap='', fig.align='center'}
...
Lets look at this phenomenon for a couple of variables in the movie dataset. As the histogram above showed, the Metacritic score variable is fairly symmetric and as a result we end up with a mean (`r round(mean(movies$metascore),1)`) and median (`r round(median(movies$metascore),1)`) that are pretty close. @fig-hist-runtime shows the distribution of movie runtime which is somewhat more right-skewed.
...
The skew here is not too large, but it pulls the mean almost three minutes higher than the median. 
...
ggplot(movies, aes(x=box_office))+
  ...
  geom_vline(data = data.frame(value=c(mean(movies$box_office),
                               ...
                               center=c("mean","median")), 
             aes(xintercept=value, color=center), size=1)+
...
I have not yet shown you how to make a boxplot using ggplot. The code for @fig-boxplot-age-sex is shown below. 
...
```{r spread-compare, echo=FALSE, fig.cap=""}
...
This example also reveals the shortcoming of the range as a measure of spread. If there are any outliers in the data, they are going to show up in the range and so the range may give you a misleading idea of how spread out the values are. Note that the 75th percentile here is only 31.28 pounds, which would suggest that the 512.3 maximum is a pretty high outlier. We can check this by graphing a boxplot, as I have done in @fig-boxplot-fare. 
