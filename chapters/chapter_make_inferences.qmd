...
As an example, lets use some data on a recent class that took this course. I will treat this class of `r sum(!is.na(students$height))` students as a population that I would like to know something about. In this case, I would like to know the mean height of the class. In most cases, the population distribution is unknown but in this case, I know the height of all `r sum(!is.na(students$height))` students because of surveys they all took at the beginning of the term. Figure @fig-hist-student-height-pop shows the population distribution of height in the class:
...
Using our popularity data, what is the mean difference in popularity (number of friend nominations) between frequent smokers and those who do not smoke frequently? 

```{r}
#| label: calculate-mean-diff
tab <- tapply(popularity$nominations, popularity$smoker, mean)
tab
