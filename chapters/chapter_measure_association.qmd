...
2. I use `geom_bar` as usual. However, if I specify this geom without an argument, the bars for each party will end up stacked on top of each other, which is not what I want. The `position="dodge"` argument tells ggplot to stack these next to each other instead (thinking of dodging shapes in Tetris).
...
#| fig-cap: Distribution of belief in anthropogenic climate change by party affiliation, ANES 2016. Here I use one of the color palettes from the Wes Anderson Paletters for Moonrise Kingdom.
